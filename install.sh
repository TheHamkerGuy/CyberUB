...
if [[ -f ".env" ]] && [[ -f "my_account.session" ]]; then
  printf "${GREEN}It seems that CyberUserbot is already installed. Exiting...${NC}\n" # skipcq
  exit
fi
...
while true; do
  ...
  1)
    ...
    printf "${GREEN}============================\\n" # skipcq
    printf "Great! CyberUserbot installed successfully and running now!\n"
...
EOL
  ...
  3)
    printf "${GREEN}============================\\n" # skipcq
    printf "Great! CyberUserbot installed successfully!\n"
